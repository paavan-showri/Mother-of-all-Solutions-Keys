...
st.set_page_config(page_title="Pareto Total Time", layout="wide")
st.title("Pareto Analysis by Total Time")
...
def make_pareto_total_time_chart(summary):
    pareto = summary.sort_values(by="Total_Time_of_Occurrence", ascending=False).reset_index(drop=True)
    total = pareto["Total_Time_of_Occurrence"].sum()
    pareto["Percent"] = (pareto["Total_Time_of_Occurrence"] / total * 100) if total > 0 else 0
    pareto["Cumulative Percent"] = pareto["Percent"].cumsum()

    labels = pareto["Activity"] + " - " + pareto["Activity Name"]

    fig, ax1 = plt.subplots(figsize=(11, 5))
    bars = ax1.bar(labels, pareto["Total_Time_of_Occurrence"])
    ax1.set_xlabel("Activity Symbol")
    ax1.set_ylabel("Total Time of Occurrence (Seconds)")
    ax1.set_title("Pareto Analysis by Total Time of Occurrence")
    ax1.tick_params(axis="x", rotation=45)

    ymax = pareto["Total_Time_of_Occurrence"].max() if not pareto.empty else 0
    offset = max(ymax * 0.01, 0.1)

    for bar in bars:
        h = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width() / 2, h + offset, f"{int(h)}", ha="center", va="bottom")
...
try:
    ...
    fig, pareto = make_pareto_total_time_chart(summary)
