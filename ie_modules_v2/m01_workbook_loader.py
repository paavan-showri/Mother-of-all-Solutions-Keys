...
EXPECTED_FPC_SHEET = "FPC_Current State"
EXPECTED_PRECEDENCE_SHEET = "Precedence Network"
EXPECTED_RESOURCE_SHEET = "Resources"
...
def load_current_state_df(file) -> pd.DataFrame:
    raw = pd.read_excel(file, sheet_name=EXPECTED_FPC_SHEET, header=None)
    header_row = _find_header_row(raw)
    file.seek(0)
    df = pd.read_excel(file, sheet_name=EXPECTED_FPC_SHEET, header=header_row)
    df.columns = [str(c).strip() for c in df.columns]
    missing = [c for c in REQUIRED_FPC_HEADERS if c not in df.columns]
    if missing:
        raise KeyError(f"Missing required headers in {EXPECTED_FPC_SHEET}: {missing}")
    return df


def load_current_state_steps(file) -> List[FPCStep]:
    df = load_current_state_df(file)
    steps: List[FPCStep] = []
...
def load_precedence_tasks(file) -> List[Task]:
    file.seek(0)
    df = pd.read_excel(file, sheet_name=EXPECTED_PRECEDENCE_SHEET)
    df.columns = [str(c).strip() for c in df.columns]
    missing = [c for c in REQUIRED_PRECEDENCE_HEADERS if c not in df.columns]
    if missing:
        raise KeyError(f"Missing required headers in {EXPECTED_PRECEDENCE_SHEET}: {missing}")
    tasks: List[Task] = []
    for _, row in df.iterrows():
...
def load_resource_capacities(file) -> Dict[str, int]:
    file.seek(0)
    df = pd.read_excel(file, sheet_name=EXPECTED_RESOURCE_SHEET)
    df.columns = [str(c).strip() for c in df.columns]
    missing = [c for c in REQUIRED_RESOURCE_HEADERS if c not in df.columns]
    if missing:
        raise KeyError(f"Missing required headers in {EXPECTED_RESOURCE_SHEET}: {missing}")
    return {
